...
| **Notifications** | Sending notifications in reaction to events. A new `NotificationType` needs three things, not two: the enum member, a template arm, **and** a `NotificationChannelRouter` route — a type missing from that map sends nothing and reports success | — | `OrderPlaced` (order confirmation), `TicketMessagePosted` (support reply), `RefundApproved`/`RefundRejected` (refund decision), `UserRegistered`/`UserProfileUpdated`/`UserInvited` |
| **Support** | Support tickets and their lifecycle: agent assignment (claim/assign/unassign, guarded by `IDistributedLock`), the append-only `SupportAuditEntry` log written in the same transaction as the change it records, the agent↔customer `TicketMessage` thread (internal notes filtered in SQL, never in a mapper), and `RefundRequest` — an agent asks, a **different** administrator decides (segregation of duties enforced in the aggregate, not by the permission), capped by the replicated order subtotal. **No money moves anywhere: the platform has no payment processing, and nothing in Orders consumes a refund event** | `SupportTicketOpenedIntegrationEvent`, `SupportTicketResolvedIntegrationEvent`, `TicketMessagePostedIntegrationEvent` (customer-visible agent messages only), `RefundRequested`/`RefundApproved`/`RefundRejectedIntegrationEvent` | `UserRegistered`, `UserProfileUpdated` (support agent replica), `OrderPlaced` (order snapshot replica — the refund ceiling); the rest of the order timeline as added |
