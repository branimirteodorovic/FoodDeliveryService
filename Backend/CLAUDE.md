...
| **Support** | Support tickets and their lifecycle: agent assignment (claim/assign/unassign, guarded by `IDistributedLock`), the append-only `SupportAuditEntry` log written in the same transaction as the change it records, the agent↔customer `TicketMessage` thread (internal notes filtered in SQL, never in a mapper), and `RefundRequest` — an agent asks, a **different** administrator decides (segregation of duties enforced in the aggregate, not by the permission), capped by the replicated order subtotal. **An approval moves real money**: Payments consumes `RefundApproved` and refunds the captured card payment, answering `Settled` or `Failed` — Support owns the authority, Payments owns the transfer | `SupportTicketOpenedIntegrationEvent`, `SupportTicketResolvedIntegrationEvent`, `TicketMessagePostedIntegrationEvent` (customer-visible agent messages only), `RefundRequested`/`RefundApproved`/`RefundRejectedIntegrationEvent` | `UserRegistered`, `UserProfileUpdated` (support agent replica), `OrderPlaced` (order snapshot replica — the refund ceiling), `RefundSettled`/`RefundFailed` (Payments' answer); the rest of the order timeline as added |
| **Payments** | Card payments through Stripe: `CustomerPaymentProfile` (saved cards), `Payment` (authorize at placement, capture on accept, release on reject/cancel), `Refund` (settles Support's approvals), `StripeEventLog` (webhook dedupe). Two rules are absolute — every mutating provider call carries an idempotency key built from a durable id (`PaymentIdempotencyKeys`), and every payment mutation takes `IDistributedLock` on `PaymentLocks.Payment(orderId)` **before** its read | `PaymentMethodAttached`/`Detached`, `PaymentAuthorized`/`PaymentAuthorizationFailed`, `PaymentCaptured`/`PaymentReleased`, `RefundSettled`/`RefundFailed` | `UserRegistered`, `OrderPlaced`/`OrderAccepted`/`OrderRejected`/`OrderCancelled`, `RefundApproved` (Support) |
...
| Provider call behind an abstraction + idempotency keys | `src/Modules/Payments/...Payments.Infrastructure/Stripe/StripePaymentGateway.cs` + `.../Application/Abstractions/Payments/PaymentIdempotencyKeys.cs` (docs: `docs/payments.md`) |
...
Each feature is specified in a `{FEATURE}_PHASE{n}_PLAN.md` at the repo root (`RESTAURANTS_PHASE1_PLAN.md`, `ORDERS_PHASE1_PLAN.md`, `NOTIFICATIONS_PHASE1_PLAN.md`, `DELIVERY_PHASE2_PLAN.md`, `REALTIME_PHASE2_PLAN.md`, `CACHING_PHASE2_PLAN.md`, `TELEMETRY_PHASE2_PLAN.md`, `KUBERNETES_PHASE2_PLAN.md`, `LOADTESTING_PHASE3_PLAN.md`, `SUPPORT_PHASE3_PLAN.md`, `HARDENING_PHASE3_PLAN.md`), broken into lettered milestones of roughly one PR each. Read the whole plan before starting a milestone, and the milestone you are building twice.
