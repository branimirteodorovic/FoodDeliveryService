...
|---|---|---|---|---|
| **Exempt** | `/health`, `/health/live`, `/health/ready`, `hubs/**`, `POST payments/webhooks/stripe` | none | not applied | The blackbox exporter probes every host every 15 s from outside and a throttled probe is a *false outage alarm*. SignalR's negotiate-then-connect is one logical connection over two requests, and a long-lived WebSocket in a concurrency slot would exhaust the global limit with clients that are idle by design. Stripe's webhook ingress is anonymous, so it partitions **by IP** — and every delivery Stripe makes comes from a small set of Stripe addresses, which means the whole provider shares one bucket: shedding it puts Stripe into exponential backoff and leaves payment state lagging reality. Its own handler verifies an HMAC before touching anything, so an unsigned flood costs a hash and a `400`. |
