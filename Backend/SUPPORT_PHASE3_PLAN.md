...
### 7.6 The reversal — Feature 3.8 Milestone H (2026-09-18)

**The decision above no longer holds, and this section is what it was traded for.** `PAYMENTS_PHASE3_PLAN.md` §10 built a Payments service, which consumes `RefundApprovedIntegrationEvent`, refunds the captured card payment at Stripe, and answers with `RefundSettled` or `RefundFailed`. `RefundStatus` gained `Settled = 3` and `Failed = 4`; `RefundRequest` gained `Settle`, `MarkFailed`, `SettledOnUtc`, `FailedOnUtc` and `FailureReason`; and Support consumes a Payments event for the first time.

**What was retracted, and what was not.** The literal claim — that no money moves — is now false and was asserted in eleven places, all of them rewritten in the same change (the list is in `PAYMENTS_PHASE3_PLAN.md` §10.3). The *reasoning* behind it survives intact, and is the reason the reversal was cheap: this plan said a real payment integration would sit **behind** an approved request rather than replace it, and that is exactly where it went. Payments performs no authority check of its own — it trusts the approval trail this module built, including the segregation of duties in §7.1 — so an aggregate written for a platform that could not pay anybody turned out to need no change to its authority rules at all.

**Three things a later milestone here has to honour.**

- **The partial unique index widened to `status IN (0, 1, 3)`** (`Add_Refund_Settlement`), and `HasActiveForOrderAsync` widened with it — they are still deliberately the same set (§7.1). `Settled` now blocks a second refund on the order where `Approved` used to, so "one order, one refund" survives the new terminal state. `Failed` does **not** block: a refund that could not be paid because the capture had not happened yet is the case where a second attempt is right. The consequence is that partial refunds are one-shot through Support, and Payments' own running-total ceiling is defence in depth rather than a routine path.
- **`Settle` and `MarkFailed` raise no domain event**, which departs from the codebase's "raise on every state change" rule. Nothing consumes one: Payments is the publisher on this leg, Notifications takes the customer's email off the Payments event directly, and the `SupportAuditEntry` written in the same transaction is the record. An event here would be an outbox row dispatched to zero handlers and a second, divergent copy of a history this module already keeps — the reasoning `SupportAuditEntry` itself is built on.
- **`ISupportAuditWriter` grew `RecordSystemAction`**, and it is the first entry in the audit log that no human performed. `Record` reads the actor off the authenticated caller, and the settlement handler runs in `ProcessInboxJob` where `ISupportContext.UserId` *throws*. The actor is `SupportAuditEntry.SystemActorId` (`Guid.Empty`) — a constant rather than a parameter, deliberately, because an audit writer that accepts an actor is one a request body could eventually reach, which is precisely what `Record`'s missing parameter exists to prevent.
...
The reference document, matching `docs/caching.md` / `docs/rate-limiting.md`: the ticket state machine as a diagram, the permission matrix, what the audit log guarantees, the projection table from §5.2, how a refund travels from an agent's request to money leaving the business (§7.6), and the analytics definitions. Link it from the root `README.md`.
