...
// API host for the Restaurants module (:5300) — reached through the YARP gateway via
// restaurants/**. Owns restaurants, menus and availability.
...
builder.Services.AddApiDocumentation(builder.Configuration, ApiDocumentation.Restaurants);
...
// (fooddeliveryservice_restaurants), the shared Redis cache and the RabbitMQ broker.
...
// endpoints, outbox/inbox job schedules (see RestaurantsModule).
...
// The OpenAPI document and the two UIs over it, at /docs/restaurants/{openapi,scalar,swagger}
