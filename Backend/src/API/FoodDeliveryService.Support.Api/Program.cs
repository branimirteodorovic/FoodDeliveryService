...
using FoodDeliveryService.Support.Api.Extensions;
using FoodDeliveryService.Support.Api.Middleware;
using FoodDeliveryService.Support.Api.OpenTelemetry;
using FoodDeliveryService.Modules.Support.Infrastructure;
using RabbitMQ.Client;
using Serilog;
using StackExchange.Redis;

// API host for the Support module (:5700) — reached through the YARP gateway via support/**.
// Ninth host, seventh module. Owns support tickets and their lifecycle; later milestones add the
// agent assignment/audit trail, the customer and order-history replicas it reads context from, and
// refund requests. It calls no other service synchronously except the permissions RPC every module
// host makes.
...
builder.Services.AddApiDocumentation(builder.Configuration, ApiDocumentation.Support);

// MediatR + FluentValidation for the module's Application assembly (commands, queries, validators).
Assembly[] moduleApplicationAssemblies = [
    FoodDeliveryService.Modules.Support.Application.AssemblyReference.Assembly];

builder.Services.AddApplication(moduleApplicationAssemblies);

// Every service gets its OWN backing stores: a dedicated PostgreSQL database
// (fooddeliveryservice_support), the shared Redis cache and the RabbitMQ broker.
...
builder.Services.AddInfrastructure(
    ...
    [SupportModule.ConfigureConsumers()],
    rabbitMqSettings,
...
// instrumentation AddInfrastructure already wired up. One call for both pillars, so a Support
// instrument cannot ship unregistered and silently record into nothing.
builder.Services.AddModuleDiagnostics(
    FoodDeliveryService.Modules.Support.Application.Diagnostics.SupportDiagnostics.Name);
...
// endpoints, outbox/inbox job schedules (see SupportModule).
...
// The OpenAPI document and the two UIs over it, at /docs/support/{openapi,scalar,swagger}
