...
internal sealed class FailOrderPaymentCommandHandler(
    ...
    : ICommandHandler<FailOrderPaymentCommand>
{
    public async Task<Result> Handle(FailOrderPaymentCommand request, CancellationToken cancellationToken)
    {
        ...
        Result result = order.FailPayment(request.Reason, request.FailedOnUtc);
