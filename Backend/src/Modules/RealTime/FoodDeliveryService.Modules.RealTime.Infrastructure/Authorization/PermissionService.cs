...
namespace FoodDeliveryService.Modules.RealTime.Infrastructure.Authorization;

/// <summary>
/// Resolves a user's module-side id and permissions for the hub handshake (called by
/// CustomClaimsTransformation on the authenticated WebSocket negotiate, just like every REST
/// request). Permissions are owned by the Users service, so this uses MassTransit request/response
/// (<see cref="IRequestClient{T}"/>) — a synchronous RPC over RabbitMQ answered by
/// GetUserPermissionsRequestConsumer in Users — with a 5-minute Redis cache in front so the bus is
/// not hit on every connection. The RealTime service enforces no permission policies of its own
/// (the hub is <c>[Authorize]</c> only), but the transformation still needs this to add the "sub"
/// claim that group membership is derived from.
/// </summary>
