...
> §8.6 records what was actually built and where it diverged — read it before §8.1–§8.5, which are the sketch.

### 8.6 What Milestone G shipped, and where it departed from §8.1–§8.5

**§8.1 — hoisting the seven copies also meant deleting a whole generator, which §8.1 did not know was there.** The hosts registered `AddOpenApi()` (Microsoft) *and* `AddSwaggerGen()` (Swashbuckle): two independent pipelines building two different documents, of which Swashbuckle's was served by **nothing at all** — `UseSwagger()` is not called anywhere in the repository, and never was. So the state before this milestone was not "a document with a bad title", it was one unserved document and one document mapped only in Development. `AddSwaggerGen` and `AddEndpointsApiExplorer` are gone; Swashbuckle stays for its Swagger UI assets, pointed at the single `Microsoft.AspNetCore.OpenApi` document. Two generators would have been two documents to keep true and two things for `OpenApiDocumentTests` to disagree about.

**The seven descriptors live in `Common.Presentation`, not in the seven `Program.cs` files.** `ApiDocumentation.Orders`, `.Restaurants`, … is a static holder of `(Slug, Title, Description)` next to the extension, and a host passes one: `AddApiDocumentation(builder.Configuration, ApiDocumentation.Orders)`. Naming services from Common looks like a layering violation and is not — these are strings, not references, and `RateLimitRoutePolicy` already keeps a table of `orders/*/accept` there for the same reason. The deciding argument is the test: descriptors built inline in seven hosts are descriptors `OpenApiDocumentTests` cannot read, so the completeness test would have been asserting against a second transcription of the same strings.

**§8.2/§8.3 — everything is served under `/docs/{slug}/`, including the OpenAPI document, and this replaces §8.2's `/scalar/v1`.** §8.3 asks for a Gateway route proxying `GET /docs/{service}` to that service's UI, and the naive reading — serve at a root path, rewrite the prefix at the edge — **does not work**: Scalar's page and Swagger UI's would come back referencing paths that are correct at the origin and 404 at the Gateway. Putting the whole surface under the same prefix in the service means the Gateway forwards `docs/{slug}/**` with an identity `PathPattern`, exactly like `orders/**`, and every URL inside the page resolves identically from either side. Concretely, per service: `/docs/{slug}/openapi/v1.json`, `/docs/{slug}/scalar`, `/docs/{slug}/swagger`, and `/docs/{slug}` redirecting to Scalar. `SecurityHeadersOptions.DocumentationPathPrefixes` already carried `/docs` from §5.1, so Milestone D's carve-out covers all of it — `OpenApiDocumentTests` asserts that rather than trusting it.

**§8.3's authorization landed in the service and not in the Gateway route, and the Gateway's docs routes are `anonymous`.** YARP's routing table is static configuration with no notion of the downstream environment, so a `default` policy on `docs/**` would also 401 a developer browsing a local compose stack — the one place the UI has to be readable, and the whole point of the route. `ApiDocumentationAuthorizationMiddleware` gates the prefix in the host instead, where the environment is known: anonymous in Development (with a startup warning), a token required everywhere else. The bar is **authentication, not a permission** — there is no `docs:read` in the seeded set and inventing one would put a Users seed migration in front of every documentation change. `GatewayRouteTests`' anonymous allow-list was widened to `{two registration paths} ∪ {docs route per documented service}`, with that reasoning written into the file; the list is still asserted in both directions, and a new documentation route is still a route someone has to justify in the same pull request. `ApiDocumentation:Enabled = false` is the stronger posture for a deployment that wants the endpoints gone rather than gated.

**Failure responses are synthesized from the verb and the route, so `.Produces` on 60 endpoints was avoided — but the *success* response could not be.** `ProblemResponseOperationTransformer` adds 400/429/500 to every operation, 404 where the route takes an id, and 409 where the verb mutates, all `$ref`-ing one `ProblemDetails` component (inlining it per response made the Orders document 68 KB; the reference makes it 32 KB). 429 is documented on the services although nothing in a service produces it: the Gateway does, every client reaches a service through the Gateway, and the Gateway serves no document — documenting it only there would document it nowhere. The success response is different: the endpoints return `IResult` from `result.Match(Results.Ok, ApiResults.Problem)`, which carries no response-type metadata at all, so nothing can infer it. Each endpoint therefore gained one line — `.Produces<T>()`, or `.Produces(StatusCodes.Status204NoContent)` for the **23** endpoints that return `Results.NoContent`. Getting that split wrong is silent, which is why the test only requires *some* 2xx and leaves which one to the author.

**The permission line is read off the endpoint metadata, never retyped.** `AuthorizationOperationTransformer` pulls each `IAuthorizeData.Policy` and appends "**Requires permission:** `orders:manage`" to the description, adds the bearer requirement, and adds 401/403 — because `EndpointAuthorizationTests` already fails the build when a policy names a permission Users does not seed, the string the document prints is a string something has checked. A hand-written one in a summary is checked by nobody.

**§8.4 — the completeness test has to *start* each host, and this is the gotcha that cost the most time.** A `WebApplication` keeps the endpoint data sources `MapEndpoints` added in its own collection and publishes them to the container only when it starts; the OpenAPI document is generated from the container's. Without `StartAsync` the document has **zero paths** and every assertion in the class passes over an empty collection — the same vacuity trap §2.1 guards, and the reason `Document_Should_DescribeTheServicesHttpSurface` runs first. Kestrel is bound on `http://127.0.0.1:0` so seven hosts start without colliding, and the documents are cached per service rather than rebuilt per assertion.
...
**For Milestone I:** the README's API section should point at `http://localhost:3000/docs/{slug}` — the Gateway, not a service port, because compose publishes ephemeral ports for the module hosts and there is no stable `:5200` to link to.
